...
{
    class FinancialRatioManagerTBK
    {
    ...
    public void Get_FinancialRatio_DataTBK()
        {
            Common.WriteLog("Process start: " + DateTime.Now.ToString());


            string qboFinancialData = "";
             qboFinancialData = "TRUNCATE TABLE Franchisor.dbo.tbl_FinancialRatioTBK;";

            QBODataProcessorTBK qbdpTBK = new QBODataProcessorTBK();
            DataTable dt = GetLocationListTBK();
            ...
            {
                QBOSettings qBOSettings = GetQBOSettingsTBK(drLocation);
                bool bQBOToken = qbdpTBK.RefreshAccessTokenTBK(qBOSettings);
                    ...
                    for (int i = 0; i < Date.Length; i++)
                    {
                        ...
                        try
                        {
                            ...
                            if (bQBOToken)
                            {
                                ...
                                {
                                    var dateRange = qbdpTBK.GetDateRange(TimePeriod);

                                    startDate = dateRange.FromDate;
                                    endDate = dateRange.ToDate;
                                    pnl = qbdpTBK.GetProfitAndLossInfoTBKs(startDate, endDate, qBOSettings);
                                    ...
                                    if (TimePeriod == "Year to last month")
                                    {
                                        ...
                                        fdReportLastYear = qbdpTBK.GetFinancialReport(qBOSettings, lastYearFromDate, lastYearToDate);
                                        YOYSales = qbdpTBK.YoYGrowth(pnl.NetEventSales, fdReportLastYear.NetEventSales);
                                        YOYIncome = qbdpTBK.YoYGrowth(pnl.NetOperatingIncome, fdReportLastYear.NetOperatingIncome);
                                    ...
                                    qboFinancialData += "Update Franchisor.dbo.tbl_FinancialRatioTBK set NetEventSalesYoYGrowthRate='" + YOYSales + "' ,NetOperatingIncomeYoYGrowthRate='" + YOYIncome + "' where LocationID='" + qBOSettings.LocationID + "';";
        ...
        public string GenerateQuery(FinancialData pnl, QBOSettings qBOSettings,string TimePeriod,bool IsMonthWise = false )
        {
            ...
            string result = "INSERT INTO Franchisor.dbo.tbl_FinancialRatioTBK (" +
        ...
        private DataTable GetLocationListTBK()
        {
        ...
        public static QBOSettings GetQBOSettingsTBK(DataRow dr)
        {
