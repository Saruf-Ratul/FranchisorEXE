...
{
    class FinancialRatioManagerDemo
    {        
        public void Get_FinancialRatio_DataDemo()
        {
            Common.WriteLog("Process start: " + DateTime.Now.ToString());

            string RevenuePerEmployeeDaily = "0";
            string qboFinancialData = "";
            qboFinancialData = " truncate table Franchisor.dbo.tbl_FinancialRatioDemo ; ";
            ...
            foreach (DataRow drLocation in dt.Rows)
            {
                ...
                try
                {
                    for (int i = 0; i < Date.Length; i++)
                    {
                        ...
                        string result = "INSERT INTO dbo.tbl_FinancialRatioDemo (FranchaisorID, LocationID, Date, QuickRatio, TotalRevenue, RevenuePerEmployee, GrossProfitMargin, ProfitMargin, GrossProfit)" +
                            "VALUES('" + 
        ...
        private DataTable GetLocationList()
        {
            ...
            string strSQL = "SELECT " +
                            ...
                            "l.LocationID, " +
                            "l.LocationName, " +
                            "l.QuickbookCompanyName, " +
                            "l.AccessToken, " +
                            "l.RefreshToken, " +
                            "l.BQOFileID, " +
                            "fg.QuickRatioDaily, " +
                            "fg.RevenuePerEmployeeDaily, " +
                            "fg.GrossProfitMarginDaily, " +
                            "fg.ProfitMarginDaily " +
                            "FROM Franchisor.dbo.tbl_Location AS l " +
                            "LEFT JOIN Franchisor.dbo.tbl_Financial_Goal AS fg " +
                            "ON fg.LocationID = l.LocationID " +
                            "WHERE l.ConnectionStatus = 1 " +
                            "AND l.FranchisorType <> 'TBK';";
