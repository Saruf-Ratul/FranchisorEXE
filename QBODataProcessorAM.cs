...
namespace FranchisorEXE
{
    ...
    public class QBODataProcessorAM
    {
        ...
        public static DataSet LoadGoalDataAM(string FranchisorID, string LocationID)
        {
        ...
        public FinancialData GetProfitAndLossInfoAM(string dateStart, string dateEnd, QBOSettings qbs)
        {
            ...
            ServiceContext context = GetServiceContextAM(qbs.AccessToken, qbs.FileID, qbs.Sandbox);
            ...
            try
            {
                ...
                Common.WriteLog("GetProfitAndLossInfoAM from QBO: " + DateTime.Now.ToString());
            }
            catch (Exception ex)
            {
        ...
        public static QBOSettings GetQBOSettingsLocationWiseAM(string FranchisorID, string LocationID)
        {
        ...
        public ServiceContext GetServiceContextAM(string accessToken, string realmId, bool QBOSandBox = false)
        {
        ...
        private T GetRowPropertyAM<T>(Row row, ItemsChoiceType1 itemsChoiceType)
        {
            int choiceElementIndex = GetChoiceElementIndexAM(row, itemsChoiceType);
            if (choiceElementIndex == -1) { return default(T); } else { return (T)row.AnyIntuitObjects[choiceElementIndex]; }
        }

        private int GetChoiceElementIndexAM(Row row, ItemsChoiceType1 itemsChoiceType)
        {
        ...
        public bool RefreshAccessTokenAM(QBOSettings qboSettings)
        {
            bool retVal = false;

            try
            {
                retVal = IsTokenValidAM(qboSettings);
        ...
        private void UpdateTokenAM(QBOSettings qboSettings)
        {
            Database db = new Database();
            db.Open();
